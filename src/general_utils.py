...
def make_color_darker(color, factor=0.7):
    ...
    darker_rgb = tuple(np.clip(rgb * factor, 0, 1))
    return mcolors.to_hex(darker_rgb)  # return string
...
def percentile_score(point, correct, data, old=False):
    if isinstance(data, int):
        answers = get_answers(data)
    else:
        answers = data
    ...
    dframe = pd.DataFrame(ctrl_distances, columns=['distance'])
    dframe['percentile'] = dframe['distance'].rank(pct=True)
    if isinstance(point, list) or isinstance(point, np.ndarray):
        res = []
        for p in point:
            perc = dframe[dframe['distance'] <= -abs(p - correct)]['percentile']
            res.append(perc.max() if not perc.empty else 0)
        return np.array(res)
    else:
        perc = dframe[dframe['distance'] <= -abs(point - correct)]['percentile']
        return perc.max() if not perc.empty else 0
...
def myboxplot(x=None,
    ...
    # mark mean dots with color if below 0
    ...
    for i, key in zip(x, data.keys()):
        ...
        if mark_means:
            ...
            if mean_val < 0:
                ax.scatter(i, mean_val, s=25, color=box_color, marker="o", edgecolor="white", zorder=5, alpha=0.9)
